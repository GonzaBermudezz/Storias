...
- `stories` agrega `image_original_url` (text nullable), `fecha_publicacion` (date nullable), `estado` (text nullable: `pendiente`, `publicando`, `publicado`, `error`), `error` (text nullable), `agregar_cta` (boolean default false) y `published_at` (timestamptz nullable). Las filas anteriores conservan estado nulo y siguen el flujo existente.
